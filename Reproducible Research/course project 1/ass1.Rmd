...
This is tehn saved in a new dataset called 'completeData'.
...
```{r, echo=TRUE, results='hide', warning=FALSE}
library(lattice)
averageStepsWeekday<- ddply(completeData,~interval,summarise,mean=mean(steps, na.rm = TRUE))
averageStepsWeekend<- ddply(completeData,~isWeekday,summarise,mean=mean(steps, na.rm = TRUE))
xyplot(steps ~ interval| isWeekday, data = completeData, type = "l", xlab = "Interval", ylab = "Number of steps", layout=c(1,2))
```